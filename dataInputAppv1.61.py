...
class Ui_MainWindow(object):
    ...
    def setupUi(self, MainWindow):
        ...
        MainWindow.setStyleSheet("background-color:{};".format(self.mainBG))
        ...
        if mode=="day":
            self.label.setStyleSheet("color: {};".format(self.textDark))
        else:
            self.label.setStyleSheet("color: {};".format(self.textLight))
        ...
        if mode == "day":
            self.dateEdit.setStyleSheet("background-color: {0}; color: {1}".format(self.dateEditBG, self.textDark))
        else:
            self.dateEdit.setStyleSheet("background-color: {0}; color: {1}".format(self.dateEditBG, self.textLight))
        ...
        self.pushButton.setStyleSheet("QPushButton{color: %s; background-color: %s; border-radius: 5px;}" ":pressed {color: black; background-color: %s;}" % (self.textLight, self.saveButtonBG, self.saveButtonPressed))
        ...
        self.pushButton_2.setStyleSheet("QPushButton{background-color: %s; border-radius: 5px;}" ":pressed{background-color: #313638; color: %s;}" % (self.textLight, self.textLight))
        ...
        self.tableWidget.setStyleSheet("background-color: {}; alternate-background-color: {}; color: {};".format(self.tableMain, self.tableAlt, self.textLight))
        ...
        for i in range(self.tableWidget.rowCount()):
            for j in range(3,7):
                ...
                if i%2 == 0:
                    self.QGroupBox.setStyleSheet("background-color: {};".format(self.tableMain))
                else:
                    self.QGroupBox.setStyleSheet("background-color: {}; border: none;".format(self.tableAlt))
        ...
        self.pushButton_3.setStyleSheet("QPushButton{background-color: %s; color: %s; border-radius: 5px;}" ":pressed{background-color: %s; color: %s}" % (self.modelButtonBG, self.modelText,self.modelButtonPressed,self.modelTextPressed))
        ...
        self.pushButton_4.setStyleSheet("QPushButton{background-color: %s; color: %s;}" % (self.modeButtonBG, self.modeButtonText))
        ...
        self.label_2.setStyleSheet("background-color: RGBA(255,255,255,0); color: {};".format(self.textLight))
    ...
    def initializeColors(self):
        ...
        self.textLight = "#E2E2E2"
        self.textDark = "#323232"

        if mode == "day":
            self.mainBG = "#AAA6A6"
            self.dateEditBG = "#B8CFCF"
            self.tableMain = "#254441"
            self.tableAlt = "#889997"
            self.saveButtonBG = "#009500"
            self.saveButtonPressed = "#43AA8B"
            self.modeButtonBG = "#1F2041"
            self.modeButtonText = "#E2E2E2"
            self.modelButtonBG = "#6E7E85"
            self.modelButtonPressed = "#EB9486"
            self.modelText = self.textLight
            self.modelTextPressed = self.modelText
        else:
            self.mainBG = "#1F2041"
            self.dateEditBG = "#4DA1A9"
            self.tableMain = "#4DA1A9"
            self.tableAlt = "#19647E"
            self.saveButtonBG = "#19647E"
            self.saveButtonPressed = "#81769C"
            self.modeButtonBG = "#FFC857"
            self.modeButtonText = "#1F2041"
            self.modelButtonBG = "#FFC857"
            self.modelButtonPressed = "#4B3F72"
            self.modelText = self.textDark
            self.modelTextPressed = self.textLight
    ...
    def changeMode(self):
        ...
        button.setStyleSheet("QPushButton{background-color: %s; color: %s;}" % (self.modeButtonBG, self.modeButtonText))

        MainWindow.setStyleSheet("background-color: {}".format(self.mainBG))

        self.tableWidget.setStyleSheet("background-color: %s; alternate-background-color: %s; color: %s;" % (self.tableMain, self.tableAlt,self.textLight))
            ...
            while i < len(groupBox):
                if i < (row*4): #If before the row to be deleted
                    for box in groupBox[i:i+4]:
                        # box.setStyleSheet("")
                        box.setStyleSheet("background-color: {};".format(self.tableMain))
                    i += 4
                    for box in groupBox[i:i+4]:
                        # box.setStyleSheet("")
                        box.setStyleSheet("background-color: {}; border: none;".format(self.tableAlt))
                    if i < (len(groupBox) - 4):
                        i+=4
    ...
    def retranslateUi(self, MainWindow):
        ...
        item.setText(_translate("MainWindow", "Final Yield"))
        ...
        if mode=="day":
            ...
            self.pushButton_4.setText(_translate("MainWindow", "Day Mode"))
    def gatherData(self):
        rows = self.tableWidget.rowCount()
        blankModel = False
            ...
            for z in range(rows):
                ...
                if model:
                    ...
                    if model.isspace() or not model:
                        blankModel = True
                    if model == "":
                        display = QMessageBox(QMessageBox.Critical,"No Model!","There is no model at row " + str(z+1) + "! Please press the 'Gather Model Names' button or enter a correct model name.")
        ...
        for y in range(rows):
            ...
            if SN:
                ...
                if model == "":
                    display = QMessageBox(QMessageBox.Warning,"No Model!","There is no model for " + str(SN.text()) + "! Please press the 'Gather Model Names' button or input a proper model name.")
                    display.setWindowIcon(QtGui.QIcon("\\\photonix04\Quality Control\\QC\Spreadsheets, Forms & Information\Import for laser status\Installer\icon-caution.ico"))
                    display.exec()
                    raise ModelError
                    return
                ...
                data[SN] = model + "," + str(c) + "," + str(v) + "," + str(lt) + "," + str(ftr) + "," + wo
