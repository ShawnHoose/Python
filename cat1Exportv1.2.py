...
class Ui_MainWindow(object):
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", "QC Mass Cat 1 Data Export v" + str(vers)))
        MainWindow.setWindowIcon(QtGui.QIcon("\\\photonix04\Quality Control\\QC\Spreadsheets, Forms & Information\Import for laser status\Installer\icon-cat1.ico"))
    ...
    def exportSlot(self):
                    ...
                    for _,_,files in os.walk(snDict[SN]): #Get WaistSolver location
                        for file in files:
                            if "WaistSolverCopyVersion" in file:
                                fileDict[file] = snDict[SN] + "\\" + file
                                # srcLoc = snDict[SN] + "\\" + file

                    if len(fileDict) > 1:
                        display = QMessageBox(QMessageBox.Warning,"Too many Waist Solvers","There are too many Waist Solvers in the folder for S/N " + SN + "! Please select the proper Waist Solver you wish to import from.")
                        display.setWindowIcon(QtGui.QIcon("\\\photonix04\Quality Control\\QC\Spreadsheets, Forms & Information\Import for laser status\Installer\icon-x.ico"))
                        display.exec()
                        ...
                        path = snDict[SN]
                        title = "Select Waist Solver"
                        srcLoc, _ = QFileDialog.getOpenFileName(dialog,title,path,filter)
        ...
        x = 1
        countMax = 100 / len(snDict) #progress bar iter helper
        ...
        columnIndex = {index: letter for index, letter in enumerate(ascii_lowercase, start=1)}
        ...
        for SN in snDict.keys():
            ...
            baseData = dataSheet["A4:AT5"].value #Copy Cat 1 data
            titles = baseData[0]
            data = baseData[1]
            copyData = {}
            i = 0
            ...
            for col in titles:
                if i < 13:
                    col = "Power (W) @ " + str(int(col)) + " kHz"
                elif 12 < i < 26:
                    col = "Pulse Width (ns) @ " + str(int(col)) + " kHz"
                if col == "Asymmetry** (Non-Valid)":
                    col = "Asymmetry"
                copyData[col] = data[i]
                i += 1

            try:
                Ui_MainWindow.dataCheck(data) #Check if data is blank
            except dataError as e:
                self.workingLabel.hide()
                return

            targetSheetHeaders = targetSheet["F3:AY3"].value

            targetRow = targetSheet["AJ" + str(targetSheet.cells.last_cell.row)].end('up').row + 1 #Target row
            index = 5
            for header in targetSheetHeaders:
                index += 1
                for key in copyData.keys():
                    if header == key:
                        cellLoc = columnIndex[index].upper() + str(targetRow)
                        targetSheet.range(cellLoc).value = copyData[key]
                    else:
                        dataPlaced = False
        ...
        if dataPlaced == False:
            display = QMessageBox(QMessageBox.Information,"Export Complete","All of the data has been exported to " + exportLoc + ". Some data has been omitted as it did not match the requested data in the Cat 1 Sheet.")
            display.setWindowIcon(QtGui.QIcon("\\\photonix04\Quality Control\\QC\Spreadsheets, Forms & Information\Import for laser status\Installer\icon.ico"))
            display.exec()
        else:
    ...
    def openDialog(self): #Open file selector dialog box
        dialog = QFileDialog(MainWindow)
        filter = "Excel Files (*.xls)"
        path = "\\\photonix04\Quality Control\QC\Cat 1 Statistical & Testing Data"
